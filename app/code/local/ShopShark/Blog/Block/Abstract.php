...
abstract class ShopShark_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = ShopShark_Blog_Helper_Data::CATEGORY_URI_PARAM;
    
    protected static $_postUriParam = ShopShark_Blog_Helper_Data::POST_URI_PARAM;
    
    protected static $_tagUriParam = ShopShark_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    {        
        ...
        foreach ($collection as $item) {
            ...
            if ($shortContent) {
                ...
                $strManager = new ShopShark_Blog_Helper_Substring(array('input' => self::$_helper->filterWYS($item->getPostContent())));
    ...
    {
        if (self::$_helper->conf(ShopShark_Blog_Helper_Data::XML_BOOKMARKS)) {
    ...
    {      
        return '<div class="blogReadmore"><a class="blog-read-more" href="'.$item->getAddress().'">'.$this->__('Read Full Post').'</a></div>';
    }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == ShopShark_Blog_Helper_Data::DEFAULT_ROOT;
    }
    ...
    protected function _prepareCollection()
    {
        if (!$this->getData('cached_collection')) {
            ...
            $collection = Mage::getModel('blog/blog')->getCollection()
                    ->addPresentFilter()
                    ->addEnableFilter(ShopShark_Blog_Model_Status::STATUS_ENABLED)
                    ->addStoreFilter()
                    ->joinComments()
            ...
            $collection->setPageSize($this->getData('perPageOverride') ? $this->getData('perPageOverride') : (int)self::$_helper->postsPerPage());
